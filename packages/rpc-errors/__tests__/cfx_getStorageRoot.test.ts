...
describe("cfx_getStorageRoot address errors", () => {
  test("invalid base32 address", async () => {
    const request = createRequest(`http://localhost:${HTTP_PORT}`);
    const error = await request<string>("cfx_getStorageRoot", ["0x"]);
...
describe("cfx_getStorageRoot epoch errors", () => {
  test("invalid params(pass number)", async () => {
    const request = createRequest(`http://localhost:${HTTP_PORT}`);
    const error = await request<string>("cfx_getStorageRoot", [
      TEST_ADDRESS,
      1,
    ]);
