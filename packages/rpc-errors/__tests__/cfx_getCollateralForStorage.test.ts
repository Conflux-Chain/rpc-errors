...
describe("cfx_getCollateralForStorage address errors", () => {
  test("invalid base32 address", async () => {
    const request = createRequest(`http://localhost:${HTTP_PORT}`);
    const error = await request<string>("cfx_getCollateralForStorage", ["0x"]);
...
describe("cfx_getCollateralForStorage epoch errors", () => {
  test("invalid params(pass number)", async () => {
    const request = createRequest(`http://localhost:${HTTP_PORT}`);
    const error = await request<string>("cfx_getCollateralForStorage", [
      TEST_ADDRESS,
      1,
    ]);
