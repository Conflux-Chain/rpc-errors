...
import { InvalidHashTypeError } from "../src/coreSpace/invalidParamsErrors/hash";
...
describe("cfx_getStorageAt address errors", () => {
  test("invalid base32 address", async () => {
    const request = createRequest(`http://localhost:${HTTP_PORT}`);
    const error = await request<string>("cfx_getStorageAt", ["0x", "0x100"]);
...
describe("cfx_getStorageAt epoch errors", () => {
  test("invalid params(pass number)", async () => {
    const request = createRequest(`http://localhost:${HTTP_PORT}`);
    const error = await request<string>("cfx_getStorageAt", [
      TEST_ADDRESS,
      "0x100",
      1,
    ]);
...
describe("cfx_getStorageAt position errors", async () => {
  test("default", async () => {
    const request = createRequest(`http://localhost:${HTTP_PORT}`);
    const error = await request<string>("cfx_getStorageAt", [TEST_ADDRESS, -1]);
